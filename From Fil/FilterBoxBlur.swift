...
public struct FilterBoxBlur: Filtering {
  ...
  public init(value: Double) {
    ...
    self.filterChain = {
      ...
      return FilterChain { (image) -> CIImage? in
        ...
        let radius = RadiusCalculator.radius(image.extent) * value / FilterBoxBlur.maximumValue
        let filter = LightRoom.Blur.BoxBlur(radius: radius)
        image >>> clamp >>> filter
